...
export default defineComponent({
  ...
  methods: {
    getWeekEvents (week, weekdays) {
      ...
      const eventsWeek = []
      eventLists.value.forEach((event, id) => {
      ...
      if (eventsWeek.length > 0) {
        ...
        infoWeek.forEach((_, i) => {
          console.log('infoWeek', _eventLists, week.length, infoWeek, i, 0, 0);
          this.insertEvent(_eventLists, week.length, infoWeek, i, 0, 0)
        })
      }
    ...
    insertEvent (eventLists, weekLength, infoWeek, index, availableDays, level) {
      const iEvent = infoWeek[index]
      if (iEvent !== undefined && iEvent.left >= availableDays) {
        // If you have space available, more events are placed
        if (iEvent.left - availableDays) {
          // It is filled with empty events
          eventLists.push({ size: iEvent.left - availableDays })
        }
        // The event is built
        eventLists.push({ size: iEvent.size, event: iEvent.event })
        ...
        eventLists.push({ size: weekLength - availableDays })
        // end of iteration
      }
    },
  ...
  setup (props) {
    onUpdated(() => {
      console.log('onUpdated', props?.events, props?.events?.length);
      if (props?.events?.length > 0) {
        getCurrentDay(10)
        // theEventLists = props.events
        // props?.events?.forEach((event, idx) => {
        //   console.log('onUpdated idx', idx, getCurrentDay(idx + 10));
        //   theEventLists.push({
        //     id: 10,
        //     title: 'Vacation',
        //     details: 'Trails and hikes, going camping! Don\'t forget to bring bear spray!',
        //     start: getCurrentDay(idx + 10),
        //     end: getCurrentDay(idx + 10),
        //     bgcolor: 'purple',
        //     icon: 'fas fa-plane'
        //   })
        // })
        selectedDate.value = today();
        selectedDate.value = '2023-09-10';
      }
    })
  }
})
